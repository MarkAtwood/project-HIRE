// hire-5s4b.41 AC: two different applications get two different pseudonyms
...
const OUT_VAR: &str = "HIRE_CHILD_OUT";
const SOCK_VAR: &str = "HIRE_CHILD_SOCK";
...
///
/// The whole token, not just the SPIFFE ID: hire-5s4b.118 is about what the
...
async fn two_consumers_get_two_pseudonyms() {
    ...
    let dir = std::env::temp_dir().join(format!("hire-twoconsumers-{pid}"));
    ...
    let handle = tokio::spawn(async move {
        hire_grpc::server::serve(&sock_for_server, service)
            .await
            .expect("server error");
    });
    ...
    // hire-5s4b.118, accepted and bounded, pinned here so nobody later
    ...
    // hire-attestors/src/claim.rs instead.
    for c in [&ca1, &cb, &ca2] {
        let attested_at = c["hire"]["presence"]["attested_at"]
            .as_u64()
            .expect("attested_at must be whole Unix seconds");
        let present_until = c["hire"]["presence"]["present_until"]
            .as_u64()
