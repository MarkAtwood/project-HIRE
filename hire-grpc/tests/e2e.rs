...
use hire_attestors::{Attestor, AttestorError, Candidate, Evidence, FreshnessResult};
...
impl Attestor for TestAttestor {
    ...
    // ssh attestor takes. Before hire-5s4b.116 this handed back the challenge
...
fn tmp_socket_path() -> String {
    ...
    format!("/tmp/hire-test-{nanos}-{seq}.sock")
}
...
/// `hire-attestors`: `observed_at` is stamped when the signature verifies,
...
/// `hire-grpc/src/service.rs` instead.
#[derive(Debug)]
...
async fn start_daemon_with(
    ...
    let handle = tokio::spawn(async move {
        hire_grpc::server::serve(std::path::Path::new(&sock_for_server), service)
            .await
            .expect("server error");
    });
...
// hire-5s4b.47: policy is per-audience, so the gate must see every audience.
...
async fn presence_requirement_is_taken_from_every_audience() {
    for order in [
        vec![
            ...
            "https://bank.example?hire_require_presence=hardware".to_owned(),
        ],
        vec![
            "https://bank.example?hire_require_presence=hardware".to_owned(),
            "https://harmless.example".to_owned(),
...
// hire-5s4b.71: a requirement the daemon does not understand is refused,
...
async fn unrecognised_presence_requirement_is_refused_not_ignored() {
    ...
    for audience in [
        "https://bank.example?hire_require_presence=biometric",
        "https://bank.example?hire_require_presence=Hardware",
    ] {
        let status = client
            .fetch_jwtsvid(JwtsvidRequest {
                audience: vec![audience.to_owned()],
            ...
            .await
            .expect_err("an unhonourable hire_ extension must not yield an SVID");
        assert_eq!(
            status.code(),
...
// The token's `aud` names the relying party, not hire's policy language.
#[tokio::test]
async fn signed_audience_has_hire_params_stripped() {
    let socket_path = tmp_socket_path();
    let (mut client, handle) = start_daemon(&socket_path).await;

    let resp = client
        .fetch_jwtsvid(JwtsvidRequest {
            audience: vec!["https://app.example?hire_require_presence=none&tenant=acme".to_owned()],
            spiffe_id: String::new(),
        })
        .await
        .expect("FetchJWTSVID RPC failed");
...
fn verify(dir: &std::path::Path, token: &str) -> std::process::ExitStatus {
    std::process::Command::new("python3")
        ...
        .arg(dir)
        .status()
        .expect("python3 is required by the hire test suite (see tests/verify_jwt_svid.py)")
}

// hire-ogiv: `hire_max_age` is enforced rather than refused. The status
...
async fn a_zero_max_age_refuses_a_claim_dated_this_instant() {
    ...
    let status = client
        .fetch_jwtsvid(JwtsvidRequest {
            audience: vec!["https://bank.example?hire_max_age=0".to_owned()],
            spiffe_id: String::new(),
        })
        .await
    ...
    assert_eq!(status.code(), tonic::Code::Unauthenticated);
    assert!(
        status.message().contains("hire_max_age"),
...
async fn a_generous_max_age_issues_a_token() {
    ...
    let resp = client
        .fetch_jwtsvid(JwtsvidRequest {
            audience: vec!["https://bank.example?hire_max_age=3600".to_owned()],
            spiffe_id: String::new(),
        })
        .await
...
async fn max_age_is_taken_from_every_audience() {
    for order in [
        vec![
            "https://harmless.example".to_owned(),
            "https://bank.example?hire_max_age=0".to_owned(),
        ],
        vec![
            "https://bank.example?hire_max_age=0".to_owned(),
            "https://harmless.example".to_owned(),
...
async fn the_token_publishes_the_observation_and_its_window() {
    ...
    let presence = &claims["hire"]["presence"];
...
async fn try_fetch(
    client: &mut SpiffeWorkloadApiClient<tonic::transport::Channel>,
    audience: &str,
) -> Result<tonic::Response<hire_grpc::workload::JwtsvidResponse>, tonic::Status> {
    client
        .fetch_jwtsvid(JwtsvidRequest {
            audience: vec![audience.to_owned()],
            spiffe_id: String::new(),
...
async fn one_observation_fails_a_tight_bound_and_passes_a_loose_one() {
    ...
    let refused = try_fetch(&mut client, "https://bank.example?hire_max_age=0")
        .await
        .expect_err("a bound that accepts nothing must not be served");
    assert_eq!(refused.code(), tonic::Code::Unauthenticated);
    assert!(
        refused.message().contains("hire_max_age"),
        "must deny at the freshness gate, got: {}",
        refused.message()
    );

    let served = try_fetch(&mut client, "https://bank.example?hire_max_age=3600")
        .await
...
// MIGRATED (hire-5s4b.116), and the migration is a move rather than a
...
// The direct end-to-end test for hire-5s4b.116: a proof answering an earlier
