...
/// ~/Library/LaunchAgents/hired.plist. Pulled from the shipped file rather
/// than copied into this binary: two copies of one config drift, and the
/// systemd pair already had.
const LAUNCHD_PLIST: &str = include_str!("../../launchd/hired.plist");

/// The systemd user unit, shared with the copy a packager installs.
const SYSTEMD_UNIT: &str = include_str!("../../systemd/hired.service");

/// Connects to the hired Unix socket and returns a ready gRPC client.
async fn connect_to_daemon() -> Result<SpiffeWorkloadApiClient<Channel>> {
    ...
    if !path.exists() {
        anyhow::bail!(
            "hired socket not found at {}\nIs hired running? Try: systemctl --user start hired",
            path.display()
        );
    }
    ...
    let channel = Endpoint::try_from("http://[::]:50051")
        ...
        .await
        .with_context(|| format!("cannot connect to hired at {}", path.display()))?;
...
async fn main() -> Result<()> {
    ...
    match subcmd {
        ...
        "--help" | "-h" => {
            ...
            println!("  install-service  write hired service file (systemd or launchd)");
            ...
            println!("  log              show how to view hired logs");
            std::process::exit(0);
        }
        other => {
            eprintln!("unknown command: {other}");
            eprintln!("run 'hire --help' for usage");
            std::process::exit(1);
        }
    }
}

/// Prints the SPIFFE identity hired reports for this caller.
///
/// A refusal is not an answer and an empty SVID list is not an identity, so
/// both exit 1. Callers gate on `hire whoami && ...`.
async fn whoami() -> Result<()> {
    ...
    let response = client
        .fetch_jwtsvid(JwtsvidRequest {
            audience: vec!["hire:whoami".to_string()],
...
/// Formats the count line for `hire enumerate`.
///
...
///
/// This runs the attestors in the CLI, not in hired. Every probe reads
/// process-local state, so the two see different worlds.
async fn enumerate() -> Result<()> {
    use hire_attestors::probe_sources;
    ...
    eprintln!("This list is what this shell can see. hired probes its own environment,");
    eprintln!("which this command does not read and cannot report: the daemon can hold");
    eprintln!("identities missing from this list and miss ones this list shows. Ask hired");
    eprintln!("directly with 'hire whoami'.");
...
async fn fetch_jwt(args: &[String]) -> Result<()> {
    ...
    match response {
        ...
        Err(status) => {
            eprintln!("error: hired: {}", status.message());
            std::process::exit(1);
        }
    }
...
async fn prove(args: &[String]) -> Result<()> {
    ...
    let audience = match audience {
        ...
        None => {
            eprintln!("error: --audience is required");
            eprintln!("usage: hire prove --audience <url> --challenge <nonce>");
            std::process::exit(1);
        }
    };
...
fn enroll(args: &[String]) -> Result<()> {
    ...
    match subcmd {
        "app" | "origin" => {
            ...
            eprintln!("There is no enrollment store. The CLI records nothing and hired keeps");
        ...
        "--help" | "-h" => {
            println!("usage: hire enroll app | origin <url>");
            std::process::exit(0);
        }
        other => {
            eprintln!("unknown enroll subcommand: {other}");
            eprintln!("usage: hire enroll app | origin <url>");
            std::process::exit(1);
        }
    }
}
...
async fn trust_bundle(args: &[String]) -> Result<()> {
    use hire_grpc::workload::JwtBundlesRequest;
    ...
    match subcmd {
        ...
        "add" => {
            eprintln!("error: trust-bundle add is not implemented (requires hired support)");
            eprintln!("No trust anchor was added. hired was not contacted.");
            std::process::exit(1);
        }
        "remove" => {
            eprintln!("error: trust-bundle remove is not implemented");
            eprintln!("No trust anchor was removed. hired was not contacted, so any anchor");
            eprintln!("for that domain is still live and tokens from it still validate.");
            std::process::exit(1);
        }
        "--help" | "-h" => {
            println!("usage: hire trust-bundle <list|add <url>|remove <domain>>");
            std::process::exit(0);
        }
        other => {
            eprintln!("unknown trust-bundle subcommand: {other}");
            eprintln!("usage: hire trust-bundle <list|add <url>|remove <domain>>");
            std::process::exit(1);
        }
    }
    Ok(())
...
/// Polls hired every 30 seconds and prints identities as they change.
///
...
/// exits 0. `hire whoami` is the health check that answers with an exit code.
///
        ...
        tokio::select! {
            _ = ticker.tick() => {
                ...
                match client.fetch_jwtsvid(JwtsvidRequest {
                    audience: vec!["hire:watch".to_string()],
                    spiffe_id: String::new(),
                }).await {
                    Ok(resp) => {
                        let svids = resp.into_inner().svids;
...
/// Prints how to read hired's log on this platform.
///
...
async fn show_log() -> Result<()> {
    println!("hired writes structured audit events to its stderr.");
    #[cfg(target_os = "macos")]
    {
        println!("launchd routes that to a file; the unified log never sees it.");
        println!("To follow it: tail -f ~/Library/Logs/hired.log");
        return Ok(());
    }
    #[allow(unreachable_code)]
    {
        if systemd_is_running() {
            ...
            println!("To follow it: journalctl --user -u hired -f");
        } else {
            println!("systemd is not running here, so there is no journal to read.");
            println!("Read the stderr of however hired was started.");
        }
        Ok(())
    }
}
...
async fn install_service() -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        ...
        println!("installed hired.plist");
        std::fs::create_dir_all(std::path::PathBuf::from(&home).join("Library/Logs"))?;
        println!("logs: ~/Library/Logs/hired.log");
        println!("run: launchctl load ~/Library/LaunchAgents/hired.plist");
        return Ok(());
    }
    #[allow(unreachable_code)]
    {
        ...
        if !systemd_is_running() {
            ...
            eprintln!("{}/hired.service", dir.display());
            print!("{SYSTEMD_UNIT}");
            std::process::exit(1);
        }
        std::fs::create_dir_all(&dir).with_context(|| format!("create {}", dir.display()))?;
        let dest = dir.join("hired.service");
        std::fs::write(&dest, SYSTEMD_UNIT).with_context(|| format!("write {}", dest.display()))?;
        println!("installed hired.service, run: systemctl --user enable --now hired");
        Ok(())
    }
}
...
mod tests {
    ...
    fn launchd_plist_is_a_template_that_substitutes_cleanly() {
        ...
        assert!(filled.contains("<string>/Users/example/.cargo/bin/hired</string>"));
        assert!(
    ...
    fn launchd_plist_logs_under_the_home_library_and_never_to_tmp() {
        ...
        assert!(filled.contains("<string>/Users/example/Library/Logs/hired.log</string>"));
    }
}
