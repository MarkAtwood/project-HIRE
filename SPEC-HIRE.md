...
| **Microsoft WAM (Web Account Manager)** | A Windows authentication broker: an application asks it for a token for "the account you signed into your Windows session" | **The closest thing to this project that exists, and it is locked down three ways.** Windows 10+ only. Microsoft Entra ID only -- the documentation says plainly it does not work with third-party identity providers. And the interface is MSAL, a Microsoft library, rather than a neutral local socket any language can call. One OS, one IdP, one SDK. `hired` intends to consume it as a source on Windows rather than compete with it. |
| **Chrome Enterprise device trust connectors** | The browser shares device signals -- device id, serial, OS version, firewall state, secure-mode -- with a third-party IdP, hardware-backed on ChromeOS | Device posture, not human identity, and scoped to the browser rather than the machine. The identity itself is still the IdP's. Same shape as AWS: the endpoint reports what it *is*, and who the user is stays a cloud-side question. |
| **AWS Verified Access / IAM Identity Center** | Cloud-side zero trust access to applications, combining workforce identity with device posture | A reverse proxy in front of applications, not an API an application calls. Identity comes from IAM Identity Center and the device signal from a browser extension plus third-party device-trust providers -- so even here the endpoint half is delegated rather than built, and a desktop program that simply wants to know who the user is has nothing to call. AWS publishes SPIRE integration guidance for App Mesh, though its own EKS workload identity is not SPIFFE -- IRSA exchanges a projected, audience-scoped, short-lived service-account JWT for credentials, and Pod Identity is an agent. The asymmetry is the point: AWS built workload identity twice, and a desktop program that wants to know who the user is still has nothing to call. IRSA's mechanism is structurally what this project does for humans. |

Surveyed across the large vendors in September 2026, the pattern is consistent: identity is
solved cloud-side, and the endpoint signal is delegated to a browser extension or a partner
agent. Microsoft is the one exception that built a real local API, and bound it to one
operating system and one identity provider. Oracle exposes SPIFFE-based *workload* identity
in OCI; OpenStack Keystone federates with SAML and OIDC and has no local agent at all.
