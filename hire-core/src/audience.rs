//! Audience string parsing with optional `hire_` extension parameters.
...
/// Parsed hire extensions extracted from an audience string.
///
/// Audience strings may carry optional structured extensions as query parameters:
/// ```text
/// https://example.com?hire_require_presence=hardware
/// ```
///
/// The `hire_` query-parameter namespace is reserved and closed: every
/// `hire_` parameter is either implemented here or rejected. Nothing in that
/// namespace is ignored, so a caller is never told yes to a requirement this
/// daemon did not evaluate. `hire_` parameters are stripped from the returned
/// `audience` field; all other query parameters are preserved.
#[derive(Debug, Clone, PartialEq)]
pub struct AudienceExtensions {
    /// The bare audience URL with `hire_` query parameters removed.
    pub audience: String,
    ...
    /// Upper bound on the age of the presence observation this audience
...
pub enum AudienceParseError {
    /// `hire_require_presence` named a level this daemon does not implement.
    #[error("unknown hire_require_presence value: {0}")]
    UnknownPresence(String),
    /// A policy name was written without its `hire_` prefix.
    #[error("{0} must be written hire_{0}; refusing to ignore a presence requirement")]
    UnprefixedExtension(String),

    /// `hire_max_age` was not a whole number of seconds.
    #[error("hire_max_age must be a whole number of seconds: {0}")]
    InvalidMaxAge(String),

    /// A `hire_` parameter this daemon does not implement was present.
    #[error("unsupported hire extension: {0}")]
    UnsupportedExtension(String),
}

/// Policy names that mean something only with the `hire_` prefix.
///
/// Written bare they would be treated as ordinary query parameters and gate
/// nothing, so they are refused rather than kept.
///
/// `max_age` keeps its place here now that the prefixed spelling gates
/// something: the bare spelling shadows a live gate, so refusing it matters
/// more, not less.
...
impl AudienceExtensions {
    /// Parse an audience string, extracting `hire_` extension parameters.
    ///
    /// `hire_` query params are stripped from the returned
    /// [`AudienceExtensions::audience`]. All other query params remain in the
    /// audience URL.
    ///
    /// A repeated `hire_require_presence` raises the requirement and never
    /// relaxes one already named, so the within-string rule is the same
    /// strictest-wins rule the caller applies across audiences.
    ///
    /// # Errors
    ///
    /// - [`AudienceParseError::UnknownPresence`] if `hire_require_presence`
    ///   names a level this daemon does not implement.
    /// - [`AudienceParseError::UnprefixedExtension`] if a policy name appears
    ///   without its `hire_` prefix, which would otherwise be kept as an
    ///   ordinary query parameter and silently gate nothing.
    /// - [`AudienceParseError::InvalidMaxAge`] if `hire_max_age` is not a
    ///   whole number of seconds. The value bounds the age of the observation
    ///   behind the claim: a claim is served only if strictly younger than the
    ///   bound, so `hire_max_age=0` accepts nothing. A repeat takes the
    ///   smallest, since for an age bound the strictest is the least.
    /// - [`AudienceParseError::UnsupportedExtension`] for any other `hire_`
    ///   parameter; the namespace stays closed.
    ...
    pub fn parse(raw: &str) -> Result<Self, AudienceParseError> {
        ...
        if let Some(q) = query {
            for param in q.split('&').filter(|s| !s.is_empty()) {
                if let Some(val) = param.strip_prefix("hire_require_presence=") {
                ...
                } else if let Some(val) = param.strip_prefix("hire_max_age=") {
                    // Seconds, matching OIDC `max_age` and RFC 7519 NumericDate.
                    // `u64::from_str` is the whole validator: it rejects "",
                    // "-1", "3.5" and anything that would overflow, so there is
                    // no saturating path to a bound of forever and no default on
                    // a malformed value.
                    let secs: u64 = val
                        .parse()
                    ...
                    // A policy name written without the `hire_` prefix. Keeping it
...
mod tests {
    ...
    fn plain_url_no_extensions() {
        ...
        assert_eq!(ext.max_age, None);
    }

    #[test]
    fn hardware_presence_is_parsed() {
        let ext = AudienceExtensions::parse("https://example.com?hire_require_presence=hardware")
            .unwrap();
        assert_eq!(ext.audience, "https://example.com");
    ...
    #[test]
    fn non_hire_params_are_kept() {
        let ext = AudienceExtensions::parse(
            "https://example.com?foo=bar&hire_require_presence=software&baz=qux",
        )
        .unwrap();
    ...
    fn unknown_presence_level_is_rejected() {
        let err = AudienceExtensions::parse("https://example.com?hire_require_presence=biometric")
            .unwrap_err();
    ...
    fn capitalised_presence_level_is_rejected() {
        let err = AudienceExtensions::parse("https://example.com?hire_require_presence=Hardware")
            .unwrap_err();
        assert!(matches!(err, AudienceParseError::UnknownPresence(_)));
    }
    ...
    fn empty_presence_value_is_rejected() {
        let err =
            AudienceExtensions::parse("https://example.com?hire_require_presence=").unwrap_err();
        assert!(matches!(err, AudienceParseError::UnknownPresence(_)));
    }
    ...
    fn repeated_presence_key_takes_the_strictest() {
        let strict_last = AudienceExtensions::parse(
            "https://example.com?hire_require_presence=none&hire_require_presence=hardware",
        )
        .unwrap();
        let strict_first = AudienceExtensions::parse(
            "https://example.com?hire_require_presence=hardware&hire_require_presence=none",
        )
        .unwrap();
    ...
    #[test]
    fn max_age_is_parsed_as_a_bound() {
        let ext = AudienceExtensions::parse("https://example.com?hire_max_age=300").unwrap();
        assert_eq!(ext.audience, "https://example.com");
        assert_eq!(ext.max_age, Some(Duration::from_secs(300)));
    }

    #[test]
    fn max_age_zero_is_parsed_not_rejected() {
        // The parser records; the gate decides what zero means.
        let ext = AudienceExtensions::parse("https://example.com?hire_max_age=0").unwrap();
        assert_eq!(ext.max_age, Some(Duration::ZERO));
    }

    #[test]
    fn non_numeric_max_age_is_rejected() {
        let err = AudienceExtensions::parse("https://example.com?hire_max_age=soon").unwrap_err();
        assert!(
            matches!(err, AudienceParseError::InvalidMaxAge(_)),
            "got {err:?}"
        );
    }
    ...
    fn negative_max_age_is_rejected() {
        // Pin it: a later slip to a signed type makes every claim fresh forever.
        let err = AudienceExtensions::parse("https://example.com?hire_max_age=-1").unwrap_err();
        assert!(
            matches!(err, AudienceParseError::InvalidMaxAge(_)),
            "got {err:?}"
        );
    ...
    #[test]
    fn overflowing_max_age_is_rejected() {
        // Not a panic, and not a saturate-to-forever.
        let err =
            AudienceExtensions::parse("https://example.com?hire_max_age=99999999999999999999")
                .unwrap_err();
        assert!(
            matches!(err, AudienceParseError::InvalidMaxAge(_)),
            "got {err:?}"
    ...
    #[test]
    fn repeated_max_age_key_takes_the_strictest() {
        let loose_last =
            AudienceExtensions::parse("https://example.com?hire_max_age=60&hire_max_age=3600")
                .unwrap();
        let loose_first =
            AudienceExtensions::parse("https://example.com?hire_max_age=3600&hire_max_age=60")
                .unwrap();
        assert_eq!(loose_last.max_age, Some(Duration::from_secs(60)));
        assert_eq!(loose_first.max_age, Some(Duration::from_secs(60)));
    }

    #[test]
    fn unknown_hire_parameter_is_rejected() {
        let err =
            AudienceExtensions::parse("https://example.com?hire_require_tpm=true").unwrap_err();
        assert!(matches!(err, AudienceParseError::UnsupportedExtension(_)));
    }

    #[test]
    fn only_hire_params_no_other_query() {
        let ext =
            AudienceExtensions::parse("https://example.com?hire_require_presence=session").unwrap();
        assert_eq!(ext.audience, "https://example.com");
