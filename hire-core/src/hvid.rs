...
/// The `hire` extension object carried in a JWT-SVID payload.
///
/// Serialises to the exact JSON shape required by the spec.
#[non_exhaustive]
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct HireClaims {
...
mod tests {
    ...
    /// Oracle: the `hire` extension printed in SPEC-HIRE.md, "HVID Extension
    ...
    fn serialises_to_the_spec_shape() {
        let value = serde_json::to_value(HireClaims::new(
            "example.com".into(),
        ...
        let obj = value.as_object().expect("hire extension is a JSON object");
    ...
    #[test]
    fn roundtrip_hire_claims() {
        let claims = HireClaims {
        ...
        let back: HireClaims = serde_json::from_str(&json).unwrap();
