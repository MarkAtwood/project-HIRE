...
pub struct SvidSigner {
    ...
    ///   hired restarts, so SPEC-HIRE.md's "correlate across sessions" holds
    ///   only within one run | upgrade path: persist these 32 bytes at 0600
    ///   alongside whatever state enrollment (hire-5s4b.103) introduces; the
...
impl SvidSigner {
    ...
    ///
    /// Exactly one key, because a hire process signs with exactly one
...
mod tests {
    ...
    fn jwt_svid_claims_are_correct() {
        ...
        let hire_val = serde_json::json!({"level": "asserted"});

        let token = signer
            .sign_jwt_svid(spiffe_id, &[audience], hire_val.clone())
            .unwrap();
        ...
        assert_eq!(claims["hire"], hire_val);
    }
    ...
    /// Signing input for [`TEST_SIG_HEX`]. Frozen: the vector is a signature over
    /// exactly these bytes, and no external oracle can regenerate it, because
    /// `sign_raw` is deterministic ECDSA (RFC 6979) and openssl's is randomised.
    /// The word here is a fixture, not the project's name -- the HIRE rename
    /// rewrote it and invalidated the vector, which is how this comment exists.
