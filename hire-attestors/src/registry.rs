...
/// load-bearing. `hire-grpc` keeps the first claim seen at the winning tier,
...
pub async fn probe_sources() -> Vec<Arc<dyn Attestor>> {
    ...
    if DidKeyAttestor::is_available() {
        ...
        tracing::debug!("did:key: HIRE_DID_KEYS not set, skipping");
    }
    ...
    if active.is_empty() {
        tracing::warn!("no identity sources available; hired will issue no SVIDs");
    } else {
