...
use hire_attestors::{Attestor, Candidate, Claim, Evidence, UnixAttestor};
...
async fn enumerate_names_the_one_account_the_oracle_names() {
    ...
    assert_eq!(
        ...
        "hired seeds a trust bundle only for ssh.local, so a candidate under \
...
async fn a_unix_account_is_iaa1_with_no_presence_because_a_uid_is_not_a_seat() {
    ...
    assert_eq!(
        ...
        "SPEC-HIRE.md:373 puts a local username at iaa1, in the same bucket as \
